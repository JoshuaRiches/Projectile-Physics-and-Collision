...
public class InterceptLauncher : MonoBehaviour {
    ...
    private void calculatePath(){
        Vec3 launchPos = new Vec3(launchPoint.transform.position); //convert from vector 3 to Vec3 custom class
    ...
    void drawPath(){
        for (int i = 0; i < pathPoints.Count-1; i++){
            Debug.DrawLine(pathPoints[i].ToVector3(), pathPoints[i+1].ToVector3(), Color.blue);
        }
    }
    ...
    /// <summary>
    /// This will create a projectile at the spawn point and will then apply the velocity, acceleration and time to the script on the projectile object
    /// </summary>
